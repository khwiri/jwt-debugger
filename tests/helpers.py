...
def load_public_key(example :Union[int, str], key_format :str =None) -> JWK:
    ...
    example_file_name   = f'public_key-example_{example}.{key_format}'
    example_key_path    = Path('.') / 'tests' / 'examples' / example_file_name
    example_key_content = example_key_path.read_text()
